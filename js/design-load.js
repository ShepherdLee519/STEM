...
function loadTasks(){
    ...
    TASKZONE.loading = true;
    
    let loading = 1;
    loading += nodes.filter(node => node.next != "").length;
    _listen("loaded", () => {
        loading--;
        if(loading == 0){
            DATA = tasksData;
            loadActivity();
            TASKZONE.loading = false;
            $(document).unbind("loaded");
        }
    });
    selectModal(tasksData["tasktype"]);
    nodes.forEach((node, index) => {
        if(node.next === "") return;
        TASKZONE.initSubTaskNodes(node.next.tasktype, index);
    });
}
