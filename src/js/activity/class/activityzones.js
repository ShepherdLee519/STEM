...
import { init } from "../activity.handlers";

export let ACTIVITYZONE = null;

export default class ActivityZones{
    constructor(tasks){
    ...
    addZones(tasktype, nodes, parentIndex = -1){
        ...
        if(!~parentIndex){
            ...
            $$.show($target.find(".design-act-zone-hassub"));
    ...
    deleteZones(parentIndex = -1){
        ...
        let $target = this.$div.children(".design-act-zone").eq(parentIndex);
        $$.hide($target.find(".design-act-zone-hassub"));
